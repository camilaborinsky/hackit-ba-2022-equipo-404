...
abstract class CashopApi {
  CashopApi(String apiUrl, {key}) : _apiHost = apiUrl;
  ...
  Future<ApiResponse<T>> get<T>({
    ...
    return dio
      ...
      options: _getRequestOptions(secured: secured),
        ...
        .then(
      (dynamic response) {
        developer.log("CashopApi :: Success :: get($url) :: $response");
      ...
      if (e is DioError) {
        ...
        developer.log("CashopApi :: Unhandled Error :: get($url) :: $e");
        throw e;
      }
    });
  }
  ...
  Future<ApiResponse<T>> post<T>({
    ...
    // developer.log(
    //     "CashopApi :: Start :: post($url) :: Excecuting with token ${getAuthToken()}");
    developer
        .log("CashopApi :: Body :: post($url) :: body => ${body.toString()}");
    ...
    return dio
      ...
      (dynamic response) {
        developer
            .log("CashopApi :: Success :: post($url) :: Response: $response");
        return ApiResponse<T>.fromJson(
            response.data as Map<String, dynamic>, mapper);
      },
      ...
      if (error is DioError) {
        ...
        developer.log("CashopApi :: Unhandled Error :: post($url) :: $error");
        throw error;
      }
    });
  }
  ...
  Future<ApiResponse<T>> delete<T>({
    ...
    // developer.log(
    //     "CashopApi :: Start :: delete($url) :: Excecuting with token ${getAuthToken()}");
    ...
    return dio
      ...
      (dynamic response) {
        developer
            .log("CashopApi :: Success :: delete($url) :: Response: $response");
        return ApiResponse<T>.fromJson(
            response.data as Map<String, dynamic>, mapper);
      },
    ).catchError((Object error) {
      if (error is DioError) {
        ...
        developer.log("CashopApi :: Unhandled Error :: delete($url) :: $error");
        throw error;
      }
    });
  }
